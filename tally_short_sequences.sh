...
grep -v "^>" $1 | awk -v max=$2 'length($0) < max' | sort|uniq > $2_or_fewer_read_length.seqs
grep -B1 -Fwf $2_or_fewer_read_length.seqs $1 | grep "^>" | cut -d "_" -f1 | sed 's/>//' | sort|uniq > $1_$2_or_fewer_read_length_samples.txt
grep -B1 -Fwf $2_or_fewer_read_length.seqs $1 | grep "^>" | cut -d "_" -f1 | awk '{count[$1]++} END {for (word in count) print word, count[word]}' | sed 's/^>//' | > $1_$2_or_fewer_read_length_sample_counts.txt
